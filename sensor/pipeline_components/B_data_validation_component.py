...
from sensor.utilities.utility_codes import util_read_yaml_file, util_write_yaml_file
...
class DataValidationcomponent:
    def __init__(
        ...
        try:
            ...
            self._schema_config = util_read_yaml_file(file_path=SCHEMA_FILE_PATH)
        except Exception as e:
            raise SensorException(e, sys)

    def sub_validate_number_of_columns(self, dataframe: DataFrame) -> bool:
        """
        ...
        """
        try:
            print("len(dataframe.columns)",len(dataframe.columns))
            print("len(self._schema_config[columns]",len(self._schema_config["columns"]))
    ...
    def sub_is_numerical_column_exist(self, df: DataFrame) -> bool:
        """
    ...
    @staticmethod
    def stat_read_data(file_path) -> DataFrame:
        try:
            return pd.read_csv(file_path)
        except Exception as e:
            raise SensorException(e, sys)

    def stat_detect_dataset_drift(
    ...
    def main_initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            train_df = DataValidationcomponent.stat_read_data(
                file_path=self.data_ingestion_artifact.trained_file_path
            )

            test_df = DataValidationcomponent.stat_read_data(
                file_path=self.data_ingestion_artifact.test_file_path
            )

            status = self.sub_validate_number_of_columns(dataframe=train_df)
            ...
            status = self.sub_validate_number_of_columns(dataframe=test_df)
            ...
            status = self.sub_is_numerical_column_exist(df=train_df)
            ...
            status = self.sub_is_numerical_column_exist(df=test_df)
            ...
            if validation_status:
                drift_status = self.stat_detect_dataset_drift(train_df, test_df)
